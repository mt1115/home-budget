...
type Product = { id: string; categoryId: string; name: string; maker: string; model: string; price: number; url: string; shop: string; image: string; sourceImage?: string; tags: string[]; memo: string; selected: boolean; favorite: boolean; color: string };
...
type ItemRow = { id: string; household_id: string; category_id: string; name: string; maker: string | null; model_number: string | null; price: number | null; url: string | null; image_url: string | null; shop_name: string | null; description: string | null; memo: string | null; status: "candidate" | "selected" | "on_hold" | "purchased" };
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      const [{ data: settings, error: settingsError }, { data: categoryRows, error: categoryError }, { data: itemRows, error: itemError }, { data: tagRows, error: tagError }, { data: itemTagRows, error: itemTagError }] = await Promise.all([
        ...
        supabase.from("items").select("id,household_id,category_id,name,maker,model_number,price,url,image_url,shop_name,description,memo,status").returns<ItemRow[]>(),
  ...
  async function saveProduct(draft: ProductDraft) {
    ...
    if (supabase && householdId) {
      const payload = { id: nextId, household_id: householdId, category_id: draft.categoryId, name: draft.name, maker: draft.maker, model_number: draft.model, price: draft.price, url: draft.url, image_url: draft.image, shop_name: draft.shop, description: draft.sourceImage ?? "", memo: draft.memo, status: draft.selected ? "selected" : "candidate", updated_at: new Date().toISOString() };
...
function ProductForm({ categories, currentType, initialCategoryId, product, allTags, onClose, onSave }: { categories: Category[]; currentType: CategoryType; initialCategoryId?: string; product: Product | null; allTags: string[]; onClose: () => void; onSave: (draft: ProductDraft) => void | Promise<void> }) {
  ...
  const [sourceImage, setSourceImage] = useState(product?.sourceImage ?? "");
  ...
  async function fillFromImage(file: File) {
    ...
    try {
      ...
      setSourceImage(await readFileAsDataUrl(uploadFile));
  ...
  async function submit(event: FormEvent) {
    ...
    await onSave({ categoryId, name: name.trim(), maker, model, price: priceNumber, url, shop, image: imageForSave, sourceImage, tags, memo, selected, favorite: selected });
  }
